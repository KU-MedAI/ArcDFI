...
class Attention(nn.Module):
    ...
    def forward(self, queries, keys, qmasks=None, kmasks=None):
        if torch.is_tensor(qmasks) and not torch.is_tensor(kmasks):
            ...
            kmasks = torch.ones(dim0,dim1).to("cuda" if torch.cuda.is_available() else "cpu")

        elif not torch.is_tensor(qmasks) and torch.is_tensor(kmasks):
            dim0, dim1 = kmasks.size(0), queries.size(1)
            qmasks = torch.ones(dim0,dim1).to("cuda" if torch.cuda.is_available() else "cpu")
        else:
            pass
...
class RFF(nn.Module):
    def __init__(self, h, d=0.1, a='ReLU'):
        super().__init__()

        self.rff = nn.Sequential(nn.Linear(h,h),nn.GELU(),nn.Linear(h,h),nn.GELU())
...
class MultiheadAttentionBlock(nn.Module):
    ...
    def forward(self, X, Y, Xm=None, Ym=None):
        if not self.clean_path:
            ...
            if self.norm_method != 'set_norm':
                H, A = self.multihead(self.norm1(X), self.norm1(Y), self.norm1(Y), Xm, Ym)
                X    = X + H
